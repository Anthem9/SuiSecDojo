...
module suisec_dojo::challenge_15_coin_accounting_mismatch;
...
public fun new_instance(owner: address, ctx: &mut TxContext): ChallengeInstance {
    ChallengeInstance { id: object::new(ctx), challenge_id: CHALLENGE_ID, owner, deposits: 0, credits: 0, solved: false }
}
...
public(package) entry fun deposit(instance: &mut ChallengeInstance, amount: u64) {
    assert!(amount > 0, EInvalidInput);
    instance.deposits = instance.deposits + amount;
    instance.credits = instance.credits + amount;
}

public(package) entry fun vulnerable_credit_without_coin(instance: &mut ChallengeInstance, amount: u64) {
    // Intentionally vulnerable: internal credits increase without a matching coin/accounting deposit.
    assert!(amount > 0, EInvalidInput);
    instance.credits = instance.credits + amount;
}
...
public(package) entry fun solve(
    ...
    assert!(instance.credits > instance.deposits, EInvalidSolution);
...
public fun deposits(instance: &ChallengeInstance): u64 { instance.deposits }
public fun credits(instance: &ChallengeInstance): u64 { instance.credits }
...
public fun destroy_for_testing(instance: ChallengeInstance) {
    let ChallengeInstance { id, challenge_id: _, owner: _, deposits: _, credits: _, solved: _ } = instance;
    object::delete(id);
}
