...
module suisec_dojo::challenge_12_shared_object_pollution;
...
public fun new_instance(owner: address, ctx: &mut TxContext): ChallengeInstance {
    ChallengeInstance { id: object::new(ctx), challenge_id: CHALLENGE_ID, owner, pollution_count: 0, solved: false }
}
...
public(package) entry fun vulnerable_pollute(instance: &mut ChallengeInstance) {
    // Intentionally vulnerable: the pollution counter has no caller or source binding.
    instance.pollution_count = instance.pollution_count + 1;
}
...
public(package) entry fun solve(
    ...
    assert!(instance.pollution_count > 0, EInvalidSolution);
...
public fun pollution_count(instance: &ChallengeInstance): u64 { instance.pollution_count }
...
public fun destroy_for_testing(instance: ChallengeInstance) {
    let ChallengeInstance { id, challenge_id: _, owner: _, pollution_count: _, solved: _ } = instance;
    object::delete(id);
}
